...
namespace CosmosDbRepositoryTest.GuidId
{
        ...
        : CosmosDbRepositoryTests<TestData<Guid>>
    {
        ...
        [TestMethod]
        public async Task FindPaged_WithContinuation_IgnoringOne_Expect_Success()
        {
            using (var context = CreateContext())
            {
                var uniqueData = Guid.NewGuid().ToString();
                ...
                var dataList = await context.Repo.FindAsync(1, null, d => d.Data == uniqueData && d.Rank < 3, q => q.OrderBy(d => d.Rank));
